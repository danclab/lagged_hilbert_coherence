...
for f_idx = 1:length(freqs)
    ...
    for l_idx = 1:length(lags)
        [num, denom, lcs(l_idx)] = lagged_hilbert_coherence(signal, srate, freq, lags(l_idx));
    end
...
for f_idx = 1:length(freqs)
    freq = freqs(f_idx);
    for l_idx = 1:length(lags)
        [num, denom, lcs(f_idx, l_idx)] = lagged_hilbert_coherence(signal, srate, freq, lags(l_idx));
    end
end
